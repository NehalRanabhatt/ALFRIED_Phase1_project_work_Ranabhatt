...
TruckCU("orders_results_19_06_2022.txt","truckInfo.txt","trip_input_sep_Apr_05_07_2022.txt","output_05_07_2022")
...
#### CU or LF - volume based calculation

truckContainer = [[L,L],[S,S,S],[L,L],[S,S,S],[L,0],[S,0,0]]
...
def TruckCU(ordersInputFile,conatinerInfoInputFile,DaytripsInputFile,outputPrefix):
    ...
    containerInfoDict = {} ##read and store the content of "containerInfoInputFile, "truckInfo.txt"
    filledOrderP2p1 = {} ####
    filledOrderP1p2 = {}
    tripDictP2p1 = {}
    tripDictP1p2 = {}
    ...
    with open(conatinerInfoInputFile) as source:
        for line in source:
            ...
            dictValue = line[1] #Max_weight_truck-->key, 8000 --> value, {"Max_weight_truck":8000, "plant2":["1501","1502","1001","1004"]}
            if "," in dictValue:
    ...
    #print(filledOrderP1p2)
    #print(tripDictP1p2)
    tripContinuedP1p2Dict = {}
    tripCancelledP1p2List = []
    #print(filledOrderP1p2)
    for dates in filledOrderP1p2:
        dictList = filledOrderP1p2[dates]
        tripTimes = tripDictP1p2[dates]
        if list(filledOrderP1p2.keys()).index(dates)<len(filledOrderP1p2)-1:
            nextDate = list(filledOrderP1p2.keys())[list(filledOrderP1p2.keys()).index(dates)+1] ##this is to satisfy the border line condition
        else:
            nextDate = "NA"
        for i in range(len(dictList)):
            ...
            if len(tripSorted)>0:
                ...
                if i<len(tripDictP1p2[dates])-1:
                    nextTripP2ReachTime = tripDictP1p2[dates][i+1][1]
                elif nextDate != "NA" :
                    nextTripP2ReachTime = tripDictP1p2[nextDate][0][1]
                else:
                    nextTripP2ReachTime = tripDictP1p2[dates][i][1]
                if tripSorted[0][0] <= tripTime[1]  or len(previous_truck_carrier_rows) == 6 or tripSorted[0][0] <= nextTripP2ReachTime:
                    tripContinuedP1p2Dict[str(tripTime[0])+" "+str(tripTime[1])] = [previous_truck_carrier_rows, previous_truck_carrier_orders, previous_truck_carrier_contId, previous_truck_carrier_latestLoad_time, previous_truck_carrier_latestDelivery_time ]
                elif len(previous_truck_carrier_rows) > 6:
                    print("ERROR truck rows cannot be more than 6")
                else:
                    tripCancelledP1p2List.append(str(tripTime[0])+" "+str(tripTime[1]))
                    for i in range(len(tripSorted)):
                        previousOrder[tripSorted[i][0]] = tripSorted[i][1]
            elif len(previousOrder) == 0:
                tripCancelledP1p2List.append(str(tripTime[0])+" "+str(tripTime[1]))
    print(previousOrder)
    #if len(previousOrder) > 0:
    #    del tripCancelledP1p2List[-1]
    #    tripContinuedP1p2Dict[str(tripTime[0])+" "+str(tripTime[1])] = [previous_truck_carrier_rows, previous_truck_carrier_orders, previous_truck_carrier_contId, previous_truck_carrier_latestLoad_time, previous_truck_carrier_latestDelivery_time ]
    previousOrder = {}
    tripContinuedP2p1Dict = {}
    tripCancelledP2p1List = []
    for dates in filledOrderP2p1:
       dictList = filledOrderP2p1[dates]
       tripTimes = tripDictP2p1[dates]
       if list(filledOrderP2p1.keys()).index(dates)<len(filledOrderP2p1)-1:
            nextDate = list(filledOrderP2p1.keys())[list(filledOrderP2p1.keys()).index(dates)+1] ##this is to satisfy the border line condition
       else:
            nextDate = "NA"
       for i in range(len(dictList)):
           trip=dictList[i]
           ...
           trip.update(previousOrder)
           previousOrder = {}
           if roundTripDict[str(tripTime[0])+" "+str(tripTime[1])] in tripCancelledP1p2List:
               trip21_cancelled = True
           else:
               trip21_cancelled = False
           tripTime=tripTimes[i]
           tripSorted=sorted(trip.items(), key=lambda t: t[0])
           if len(tripSorted) > 0:
               truck_carrier_rows, truck_carrier_orders, truck_carrier_contId, truck_carrier_latestLoad_time, truck_carrier_latestDelivery_time = countFilledRows(tripSorted)
               if len(truck_carrier_rows) >= 6:
                   ...
                   previous_truck_carrier_rows, previous_truck_carrier_orders, previous_truck_carrier_contId, previous_truck_carrier_latestLoad_time, previous_truck_carrier_latestDelivery_time = truck_carrier_rows, truck_carrier_orders, truck_carrier_contId, truck_carrier_latestLoad_time, truck_carrier_latestDelivery_time
               if i<len(tripDictP2p1[dates])-1:
                    nextTripP1ReachTime = tripDictP2p1[dates][i+1][1]
               elif nextDate != "NA" :
                    nextTripP1ReachTime = tripDictP2p1[nextDate][0][1]
               else:
                    nextTripP1ReachTime = tripDictP2p1[dates][i][1]
               if len(previous_truck_carrier_rows)==6 or tripSorted[0][0] <= tripTime[1] or tripSorted[0][0] <= nextTripP1ReachTime:
                   if trip21_cancelled == True:
                       del tripCancelledP1p2List[tripCancelledP1p2List.index(roundTripDict[str(tripTime[0])+" "+str(tripTime[1])])]
                       tripContinuedP1p2Dict[roundTripDict[str(tripTime[0])+" "+str(tripTime[1])]] = [[], [], [], [], []]
                   tripContinuedP2p1Dict[str(tripTime[0])+" "+str(tripTime[1])] = [previous_truck_carrier_rows, previous_truck_carrier_orders, previous_truck_carrier_contId, previous_truck_carrier_latestLoad_time, previous_truck_carrier_latestDelivery_time ]
               else:
                   if trip21_cancelled == False:
                           tripContinuedP2p1Dict[str(tripTime[0])+" "+str(tripTime[1])] = [previous_truck_carrier_rows, previous_truck_carrier_orders, previous_truck_carrier_contId, previous_truck_carrier_latestLoad_time, previous_truck_carrier_latestDelivery_time ]
                   elif trip21_cancelled == True:
                       tripCancelledP2p1List.append(str(tripTime[0])+" "+str(tripTime[1]))
                       previousOrder = makeDict(previous_truck_carrier_rows, previous_truck_carrier_orders, previous_truck_carrier_contId, previous_truck_carrier_latestLoad_time, previous_truck_carrier_latestDelivery_time)
           else:
               if trip21_cancelled == False:
                   tripContinuedP2p1Dict[str(tripTime[0])+" "+str(tripTime[1])] = [[], [], [], [], []]
    ...
    dest2.write("tripStartDate"+" "+"tripStartTime"+" "+"tripEndDate"+" "+"tripEndTime"+" "+"total_containers"+" "+"trip_cus"+" "+"filledOrderIds"+" "+"emptyOrderIds"+" "+"filledcontainers"+" "+\
                "emptyContainers"+"\n")
    dest3 = open(outputPrefix+"_cancelled_trips_info.txt","w")
    ...
    for tripP2p1 in roundTripDict:
        if tripP2p1 in tripContinuedP2p1Dict:
            ...
            if tripP1p2 in tripContinuedP1p2Dict:
                ...
                print("ERROR for the trip P1p2 ", tripP2p1)
        elif roundTripDict[tripP2p1] in tripContinuedP1p2Dict:
                print("ERROR for the trip P2p1 ", tripP1p2)
        else:
            dest3.write(tripP2p1+"\t"+roundTripDict[tripP2p1])
            dest3.write("\n")
...
def adjustCarrierRows(fillRow, fillOrders, fillContId, fillLoadTime, fillDeliveryTime, truck_carrier_rows, truck_carrier_orders, truck_carrier_contId, truck_carrier_latestLoad_time, truck_carrier_latestDelivery_time, container_type_divide):
    ...
    #dest7.write(emptySpaceGlobalIndex, emptySpaceLocalIndex, emptySpaceRowLength, emptySpaceEmptyContainerPresent)
        #print(emptySpaceRowLength)
        #print(emptySpaceEmptyContainerPresent)
        ...
        for j in range(len(fillRow)):
            if len(fillRow[j]) == emptySpaceRowLength[i]:
                ...
                if emptySpaceEmptyContainerPresent[i] == presentEmptyOrder:
                    if 1 in fillRow[j]:
                        containerIndex = len(fillRow[j])-1-fillRow[j][::-1].index(1) ##indexing from reverse
